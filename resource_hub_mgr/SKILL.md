...
The wrapper bootstraps and reuses `.venv` under the skill directory, so future Python dependencies stay isolated from the system environment.
...
Within a single script process, LLM provider clients should be reused via shared helper code instead of being recreated ad hoc in each call path.
