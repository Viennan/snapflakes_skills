...
```mixin-react
return (<Tabs>
<Tabs.TabPane title="多模态混合输入" key="it6eFdGfel"><RenderMd content={`> * doubao\\-embedding\\-vision\\-250615 及后续版本支持 **不限数量的视频、文本和图片混合** 输入。
...
<Tabs.TabPane title="开启稀疏向量输出" key="NOuUo9lLGW"><RenderMd content={`> *  doubao\\-embedding\\-vision\\-250615及后续版本支持。
> * 支持纯文本输入。
...
# 支持模型
当前支持多模态向量化的模型请参见[向量化能力](/docs/82379/1330310#ee5ec35c)。
不同模型版本的功能对比如下：
...
<span>![图片](https://p9-arcosite.byteimg.com/obj/tos-cn-i-goo7wpa0wc/592fdf8b3e374e788ff84ec9329089ef =1462x) </span>
...
|**场景** |**细分场景** |**业务需求场景** |场景举例 |
|---|---|---|---|
|<div style="text-align: center">|\\- |需要对大量数据进行离线处理以转换为向量表示时，使用多模态向量化模型进行批量请求处理。 |批量处理电子商务平台上的商品图片，将其转换为向量表示以便于相似性匹配。 |\
|批量数据向量化处理</div>| | | |\
| | | | |
|<div style="text-align: center">|文搜图 |在图库场景中，用户侧重以文本描述来检索符合描述的图片。 |用户输入“蓝色海景”，系统根据描述搜索并展示相关蓝色海景图片。 |\
|文本描述检索</div>| | | |\
| | | | |
|^^|文搜文 |用于在知识库中进行文本描述检索，找到相关文档或知识库内容。 |用户输入“如何重置密码”，系统检索知识库中包含密码重置方法的文档。 |
|<div style="text-align: center">|图搜图 |图片检索相似物体。通过用户提供的图片，在不同场景下检索同一类物体，例如用于商品图搜索。 |用户上传一张椅子图片，系统返回相似款式的椅子商品图片。 |\
|图片特征检索</div>| | | |\
| | | | |
